...
class GeocodeCache:
    ...
    def geocode_address(self, address, code_postal=None):
        ...
        # Normalize CP to 4-5 digits string if provided
        cp_part = ""
...
def load_site_addresses():
    """
    Load site addresses from /mnt/data/villes-afe.docx if available (python-docx),
    else return a built-in fallback list. Addresses are suffixed with ', France'.
    """
...
def process_excel_data(file):
    ...
    for c in df.columns:
        ...
        # Amélioration de la détection pour code_dossier
...
def apply_sidebar_filters(df, cmap):
    """Apply cumulative AND filters from sidebar directly to df and return filtered df and dict of selections."""
    filters_selected = {}
    ...
    # Ajout du filtre pour code_dossier
...
# Map creation
# =========================
def create_map(df, cmap, geocache):
    # Aggregate by (ville, CP)
    ...
    # Build dict counts + build text with up to 15 client names and their code_dossier
    counts = {}
    clients_per_key = {}
    ...
    for _, row in df.iterrows():
        ...
        if key not in clients_per_key:
            clients_per_key[key] = []
            codes_dossier_per_key[key] = set()  # Utilisation d'un set pour éviter les doublons
        ...
        clients_per_key[key].append(client_info)

    # Fonction pour déterminer la couleur selon le nombre de clients
    def get_color_by_count(nb_clients):
        if nb_clients <= 10:
            return 'rgba(0, 128, 0, 0.7)'  # Vert transparent
        elif nb_clients <= 49:
            return 'rgba(50, 130, 184, 0.7)'  # Bleu transparent
        elif nb_clients <= 100:
            return 'rgba(255, 140, 0, 0.7)'  # Orange transparent
        else:
            return 'rgba(255, 0, 0, 0.7)'  # Rouge transparent

    # Fonction pour déterminer la catégorie textuelle
    ...
    # Grouper par catégorie pour créer des traces séparées (pour la légende)
    ...
    # Sites as red targets (optional toggle)
    ...
    fig.update_layout(
        ...
        showlegend=True,
        # Configuration pour permettre le zoom et l'interaction
        dragmode='pan'
    )
    return fig, mdf
...
def analyze_proximity(df_filtered, cmap, geocache, radius_km=50):
    """
    Analyse la proximité entre clients et sites
    """
    if 'ville' not in cmap:
        return None, None
    
    # Récupérer les coordonnées des sites
    site_coords = []
    ...
    # Calculer les distances pour chaque groupe de clients
    proximity_data = []
    zones_sans_site = []
    
    for (ville, cp), nb_clients in client_groups.items():
        # Géocoder la ville des clients
        ...
        # Déterminer si c'est dans le rayon ou une zone sans site
        ...
        data_point = {
            ...
            'site_proche': closest_site['adresse'] if closest_site else "",
            'distance_km': round(min_distance, 1),
            'dans_rayon': within_radius
        }
        
        if within_radius:
            proximity_data.append(data_point)
        else:
            zones_sans_site.append(data_point)
    
    return proximity_data, zones_sans_site

def create_proximity_map(proximity_data, zones_sans_site, geocache, radius_km=50):
    """
    Crée une carte d'analyse de proximité
    """
    fig = go.Figure()
    ...
    # Afficher les sites avec cercles de rayon
    if site_coords:
        site_lats = [x[0] for x in site_coords]
        site_lons = [x[1] for x in site_coords]
        site_texts = [x[2] for x in site_coords]
        
        # Cercles de rayon autour des sites
        for lat, lon, addr in site_coords:
            # Approximation du rayon en degrés (1° ≈ 111 km)
    ...
    if site_coords:
        all_lats.extend([x[0] for x in site_coords])
        all_lons.extend([x[1] for x in site_coords])
    if proximity_data:
        all_lats.extend([x['lat'] for x in proximity_data])
        all_lons.extend([x['lon'] for x in proximity_data])
    if zones_sans_site:
        all_lats.extend([x['lat'] for x in zones_sans_site])
        all_lons.extend([x['lon'] for x in zones_sans_site])
    
    if all_lats:
        center_lat = np.mean(all_lats)
        center_lon = np.mean(all_lons)
    else:
        center_lat, center_lon = 44.0, 4.0
...
def main():
    ...
    for logical, label in [
        ...
        ('code_dossier','Code dossier'),  # Ajout du code dossier dans l'affichage
    ...
    if cols_to_show:
        ...
        # Sort by Ville then Nom if available
    ...
    # Analyse de proximité Sites/Clients
    st.markdown("---")
    ...
    if st.button("🎯 Lancer l'analyse de proximité"):
        ...
        # Carte de proximité
        prox_fig = create_proximity_map(proximity_data, zones_sans_site, geocache, radius_km)
        ...
        with col1:
            ...
            if proximity_data:
                prox_df = pd.DataFrame(proximity_data)
                prox_df = prox_df.sort_values('nb_clients', ascending=False)
                prox_display = prox_df[['ville', 'cp', 'nb_clients', 'distance_km', 'site_proche']].copy()
                prox_display.columns = ['Ville', 'CP', 'Nb Clients', 'Distance (km)', 'Site le plus proche']
                st.dataframe(prox_display, use_container_width=True)
                
                csv_prox = prox_display.to_csv(index=False, sep=';').encode('utf-8')
                st.download_button("💾 Exporter zones couvertes", data=csv_prox, file_name="zones_couvertes.csv", mime="text/csv")
            else:
        ...
        with col2:
            ...
            if zones_sans_site:
                zones_df = pd.DataFrame(zones_sans_site)
                zones_df = zones_df.sort_values('nb_clients', ascending=False)
                zones_display = zones_df[['ville', 'cp', 'nb_clients', 'distance_km', 'site_proche']].copy()
                zones_display.columns = ['Ville', 'CP', 'Nb Clients', 'Distance (km)', 'Site le plus proche']
                st.dataframe(zones_display, use_container_width=True)
                
                csv_zones = zones_display.to_csv(index=False, sep=';').encode('utf-8')
                st.download_button("💾 Exporter zones d'opportunité", data=csv_zones, file_name="zones_opportunite.csv", mime="text/csv")
            else:
    ...
    if st.button("🔍 Lancer l'analyse"):
        with st.spinner("Analyse en cours…"):
            vdf, diag = analyze_concentration(df_filtered, cmap, geocache) or (None, None)
        if vdf is None:
            st.warning("Impossible de créer la carte de concentration (aucun groupe géocodé).")
            if diag and diag.get('failed_groups'):
                st.write("Groupes non géocodés :", pd.DataFrame(diag['failed_groups']))
            return
        ...
        if diag.get('failed_groups'):
            with st.expander("Voir les groupes non géocodés"):
                fg = pd.DataFrame(diag['failed_groups'])
                fg = fg.rename(columns={'ville':'Ville','code_postal':'Code postal','nb_clients':'Nombre de clients'})
                st.dataframe(fg, use_container_width=True)

        cfig = concentration_map(vdf)
