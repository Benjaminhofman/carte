...
def apply_sidebar_filters(df, cmap):
    ...
    if 'ville' in cmap and cmap['ville'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Ville", villes, placeholder="Sélectionner")
        if sel:
    ...
    if 'code_postal' in cmap and cmap['code_postal'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Code postal", cps, placeholder="Sélectionner")
        if sel:
    ...
    if 'site' in cmap and cmap['site'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Site", sites, placeholder="Sélectionner")
        if sel:
    ...
    if 'responsable' in cmap and cmap['responsable'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Responsable", responsables, placeholder="Sélectionner")
        if sel:
    ...
    if 'collaborateur' in cmap and cmap['collaborateur'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Collaborateur", collabs, placeholder="Sélectionner")
        if sel:
    ...
    if 'activite' in cmap and cmap['activite'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Secteur d'activité", acts, placeholder="Sélectionner")
        if sel:
            df = df[df[cmap['activite']].astype(str).isin(sel)]
            filters_selected['activite'] = sel


    # --- Filtres prédéfinis Santé ---
    if 'activite' in cmap and cmap['activite'] in df.columns:
        sante_keywords = [
            "santé", "médical", "médicaux", "médecin", "soins", "hospital", "clinique",
            "dent", "dentaire", "kiné", "rééducation", "pharma", "sage-femme", "infirm",
            "psy", "radiolog", "ophtalm", "chirurg", "analyses", "biolog", "vétérin"
        ]

        incl_sante = st.sidebar.checkbox("✅ Inclure uniquement les activités santé")
        excl_sante = st.sidebar.checkbox("🚫 Exclure les activités santé")

        if incl_sante and excl_sante:
            st.sidebar.warning("⚠️ Choisissez seulement un filtre Santé (inclusion ou exclusion).")
        elif incl_sante:
            df = df[df[cmap['activite']].astype(str).str.lower().str.contains("|".join(sante_keywords), na=False)]
            filters_selected['activite_sante'] = "inclusion"
        elif excl_sante:
            df = df[~df[cmap['activite']].astype(str).str.lower().str.contains("|".join(sante_keywords), na=False)]
            filters_selected['activite_sante'] = "exclusion"

    # --- Filtres prédéfinis Location ---
    location_activities = {
    ...
    location_formes = {"SC", "SCI", "SCP", "GFA"}

    incl_loc = st.sidebar.checkbox("🏠 Inclure uniquement les activités de location (ou SC/SCI/SCP/GFA)")
    excl_loc = st.sidebar.checkbox("🚫 Exclure les activités de location (et SC/SCI/SCP/GFA)")

    if incl_loc and excl_loc:
        st.sidebar.warning("⚠️ Choisissez seulement un filtre Location (inclusion ou exclusion).")
    elif incl_loc:
        mask_activite = df[cmap['activite']].astype(str).isin(location_activities) if 'activite' in cmap and cmap['activite'] in df.columns else False
        mask_forme = df[cmap['forme']].astype(str).isin(location_formes) if 'forme' in cmap and cmap['forme'] in df.columns else False
        df = df[mask_activite | mask_forme]
        filters_selected['location'] = "inclusion"
    elif excl_loc:
        mask_activite = df[cmap['activite']].astype(str).isin(location_activities) if 'activite' in cmap and cmap['activite'] in df.columns else False
        mask_forme = df[cmap['forme']].astype(str).isin(location_formes) if 'forme' in cmap and cmap['forme'] in df.columns else False
        df = df[~(mask_activite | mask_forme)]
        filters_selected['location'] = "exclusion"



    if 'forme' in cmap and cmap['forme'] in df.columns:
        formes = sorted(df[cmap['forme']].dropna().astype(str).unique())
        sel = st.sidebar.multiselect("Forme juridique", formes, placeholder="Sélectionner")
        if sel:
    ...
    if 'code_dossier' in cmap and cmap['code_dossier'] in df.columns:
        ...
        sel = st.sidebar.multiselect("Code dossier", codes, placeholder="Sélectionner")
        if sel:
...
def main():
    ...
    if cols_to_show:
        ...
        with pd.ExcelWriter(excel_buffer, engine="xlsxwriter") as writer:
            disp.to_excel(writer, index=False, sheet_name="Clients")
        excel_buffer.seek(0)
        st.download_button("📘 Exporter la liste (Excel)", data=excel_buffer.getvalue(), file_name="clients_filtres.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    else:
